...
class MainFrame(wx.Frame):
	...
	def update(self):
		...
		self.txtctrl_filename.Enable(enabled)
		self.txtctrl_size.Enable(enabled)
		self.txtctrl_md5.Enable(enabled)
		self.txtctrl_sha1.Enable(enabled)
		# Status text
	...
	def onLicenseSelected(self, evt):
		name = self.combo_license_name.GetValue()
	...
	def onMenuOpen(self, evt):
		...
		if filename != "":
			...
			for res in self.ml.resources:
				...
				self.filelist.SetStringItem(num_items, 2, res.preference)
				self.filelist.SetStringItem(num_items, 3, res.conns)
			if len(self.ml.pieces) > 0 or self.ml.hash_md5 != "" or self.ml.hash_sha1 != "":
				self.locked = True
			else:
				self.locked = False
			self.new_file = False
...
class MainApp(wx.App):
	def OnInit(self):
		if wx.GetOsDescription().find("Windows") == -1:
			...
			print "Not on win32. Using configuarion file:", configname
		else:
			configname = "metalink_editor.ini"
			print "Running win32. Using configuarion file:", configname
