...
%JYTHON_HOME%\jython.bat setup.py clean
...
cd jyinterface
%JAVA_HOME%\bin\javac.exe -classpath %JYTHON_HOME%\jython.jar Download.java JythonFactory.java Main.java
...
REM copy GPG.py %BUILD_DIR%\.
copy xmlutils.py %BUILD_DIR%\.

cd %BUILD_DIR%
echo Class-Path: jython.jar > manifest.txt
%JAVA_HOME%\bin\jar.exe cvfme applet.jar manifest.txt jyinterface.Main *
